...
| `attached_data_sources` | array[Data source id] |  | List of data sources equipping this device |
...
| `attached_data_sources` | array[Data source id] |  | List of data sources equipping this stereo rig |
...
| `capacity` | <a href="#unit">Unit</a> |  | load capacity |
| `attached_data_sources` | array[Data source id] |  | List of data sources (extensometers, load cells) equipping this device |
...
### full displacement field measurement (`data_sources/dic_measurement`)

subset/global DIC, DVC, Stereo, optical flow
...
| `model` | string |  | Software version. |
| `documentation` | string |  | Documentation filename, path or URL |
| `parameters` | array[<a href="#unit">Unit</a>] |  | list of constitutive, geometric, loading or numerical parameters  |
...
When all the timestamps and data (number or filenames) is stored in a single spreadsheet like file (like csv or txt). If the timestamps and data are in two separate files they should be in the same folder.
